...
// JSON-string loader for wasm builds: there is no filesystem there, so the
// model file contents are passed in from JavaScript as a string.
std::unique_ptr<DSP> get_dsp(const char* jsonStr)
{
  nlohmann::json j = nlohmann::json::parse(jsonStr);
  return get_dsp(j, DspLoadOptions());
}
